...
public class DoctorService {
    ...
    /*按照查询条件分页查询医生信息记录*/
    public ArrayList<Doctor> queryZxDoctor(int number) throws Exception { 
     	String where = "where 1=1";
    	 
    	return doctorMapper.queryZxDoctor(where, number);
    }
	...
	public boolean checkLogin(String userName, String password) throws Exception {
		Doctor db_doctor = (Doctor) doctorMapper.getDoctor(userName);
		if(db_doctor == null) { 
			this.errMessage = " 账号不存在 ";
			System.out.print(this.errMessage);
			return false;
		} else if( !db_doctor.getPassword().equals(password)) {
